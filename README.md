...
把散乱的 JAV 影片文件整理成带封面和元数据的有序收藏，并生成可离线浏览的片库页面。
...
一个 JAV 影片本地整理工具。你把下载好的影片放进一个目录，它会：
...
只处理**正规厂商发行的 JAV 影片**（「字母-数字」固定番号，如 `XXX-000`），数据来自 javbus，单源不聚合。
